...
int main() {
    ...
    // Create DB
    // initdb -D /Users/alandefreitas/pgsql_data/
    // Start server
    // postgres -D /Users/alandefreitas/pgsql_data/

    //connector db({{"db_name","smartlao"},{"data_folder","/Users/alandefreitas/Sync/Computação/Code/C++/Web/wpp/tools/website/model/data/"}});
    //
    //if (!db.is_open()){
    //    return 1;
    //}
    //
    //result R = db.select("SELECT * FROM INFORMATION_SCHEMA.TABLES WHERE table_schema = 'public'");
    //if (!db.error()) {
    //    for (row row: R){
    //        for (field &&field : row) {
    //            std::cout << field.name() << ": ";
    ...
    //    }
    //} else {
    //    std::cout << db.error_string() << std::endl;
    //}
    //
