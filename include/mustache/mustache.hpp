    ...
    namespace mustache {
        ...
        class basic_lambda_t {
            public:
                ...
                using type2 = std::function<string_type(const string_type &,
                                                        basic_renderer<string_type> &render)>;
                ...
                string_type operator()(const string_type &text, basic_renderer<string_type> &render) const {
        ...
        class basic_data {
            public:
                ...
                void merge(basic_data &d_right){
                    // put right elements over left elements
                    // merge if it's the same type
                    if (d_right.is_list() && this->is_list()){
                        for (int i = 0; i < std::min(this->size(),d_right.size()); ++i) {
                            (*(this->list_))[i].merge((*(d_right.list_))[i]);
                        }
                        for (int i = this->size(); i < d_right.size(); ++i) {
                            this->list_->push_back(std::move((*(d_right.list_))[i]));
                        }
                    } else if (d_right.is_object() && this->is_object()) {
                        // for each component of the object on the right
                        for (typename basic_object<string_type>::iterator iter_component = d_right.obj_->begin();
                             iter_component != d_right.obj_->end(); ++iter_component) {
                            // check if the component exists on the left
                            typename basic_object<string_type>::iterator component_left_value = this->obj_->find(iter_component->first);
                            if (component_left_value != this->obj_->end()){
                                // merge right hand side if it exists
                                component_left_value->second.merge(iter_component->second);
                            } else {
                                // just insert if it doesn't
                                this->obj_->insert(std::make_pair(iter_component->first,iter_component->second));
                            }
                        }
                    } else {
                        // replace
                        *this = std::move(d_right);
                    }
                }
                ...
                std::string dump(int level = 0) const {
                    ...
                    // merge if it's the same type
                    if (this->is_list()){
                        result += std::string(level,' ') + "list[\n" + std::string(level+4,' ') ;
                        for (const basic_data& item: *(this->list_)){
                            result += item.dump(level+4) + ", ";
                        }
                        result += "]";
                    } else if (this->is_object()) {
                        result += std::string(level,' ') + "object[\n";
                        // for each component of the object on the right
                        for (auto iter_component = this->obj_->begin(); iter_component != this->obj_->end(); ++iter_component) {
                             result += std::string(level+4,' ') + "\"" + iter_component->first + "\": " + iter_component->second.dump() + ", \n";
                        }
                        result += std::string(level,' ') + "]";
                    } else {
                        // replace
                        if (this->is_string()){
                            result += "\"" + this->string_value() + "\"";
                        } else if (this->is_bool()) {
                            result += this->is_true() ? "true" : "false";
                        }
                    }
                    return result;
                }
        ...
        template<typename string_type>
        const string_type delimiter_set<string_type>::default_begin(

        2, '{');

        template<typename string_type>
        const string_type delimiter_set<string_type>::default_end(
        ...
        class basic_context {
            public:
                ...
                virtual void push_copy(const basic_data<string_type> *data) = 0;

                virtual void pop() = 0;

                virtual const basic_data<string_type> *get(const string_type &name) const = 0;

                virtual const basic_data<string_type> *get_partial(const string_type &name) = 0;
        };

        template<typename string_type>
        class context
                : public basic_context<string_type> {
            public:
        ...
        class basic_mustache {
            public:
                ...
                std::vector<string_type> get_dependencies(){
                    std::vector<string_type> partial_list;
                    get_dependencies(this->rootComponent_,partial_list);
                    return partial_list;
                }
                ...
                class Tag {
                    public:
                        ...
                        bool isSectionBegin() const {
                            return type == Type::SectionBegin || type == Type::SectionBeginInverted || type == Type::InheritanceBegin || type == Type::ContentBegin || type == Type::LambdaBegin;
                        }
                ...
                void get_dependencies(mustache::basic_mustache<string_type>::component& comp, std::vector<string_type>& partial_list){
                    if (comp.tag.type == Tag::Type::Partial) {
                        // push string to vector
                        partial_list.push_back(comp.tag.name);
                    } else if (comp.tag.type == Tag::Type::InheritanceBegin) {
                        // push string to vector
                        partial_list.push_back(comp.tag.name);
                    } else {
                        // iterate children
                        for (component& child: comp.children){
                            child.get_dependencies(comp,partial_list);
                        }
                    }
                }
                ...
                class context_internal {
                    public:
                        ...
                        std::vector<std::unordered_map<string_type,component>*> inherited_content;
                        std::unordered_map<string_type,string_type> stacks;
                ...
                : public basic_renderer<string_type>
                {
                    public:
                        using type1 = std::function<string_type(const string_type &)>;
                        using type2 = std::function<string_type(const string_type &, bool escaped)>;

                        renderer_internal(const type1 &t1, const type2 &t2):basic_renderer<string_type>(t1,t2) {}
                        renderer_internal(const type1 &t1, const type2 &t2, context_internal* ctx_):basic_renderer<string_type>(t1,t2),ctx(ctx_){}

                        context_internal* ctx;
                };
            ...
            private:
                ...
                void parse(const string_type &input, context_internal &ctx) {
                    ...
                    while (inputPosition != inputSize) {
                        ...
                        if (tagLocationStart == string_type::npos) {
                            ...
                            const component comp{{input, inputPosition, tagLocationStart - inputPosition},
                                                 inputPosition};
                            sections.back()->children.push_back(comp);
                        }
                        ...
                        const bool tagIsUnescapedVar{currentDelimiterIsBrace && tagLocationStart != (inputSize - 2) &&
                                                     input.at(tagContentsLocation) == ctx.delimiterSet.begin.at(0)};
                        const string_type &currentTagDelimiterEnd{
                                tagIsUnescapedVar ? braceDelimiterEndUnescaped : ctx.delimiterSet.end};
                        ...
                        // Parse tag
                        const string_type tagContents{
                                trim(string_type{input, tagContentsLocation, tagLocationEnd - tagContentsLocation})};
                        component comp;
                        ...
                        if (comp.tag.isSectionBegin()) {
                            ...
                            sections.back()->tag.sectionText.reset(new string_type(
                                    input.substr(sectionStarts.back(), tagLocationStart - sectionStarts.back())));
                            sections.pop_back();
                    ...
                    walk([this](component &comp) -> WalkControl {
                        ...
                        if (comp.children.empty() || !comp.children.back().tag.isSectionEnd() ||
                    ...
                    if (isUnescapedVar) {
                        ...
                        switch (contents.at(0)) {
                            ...
                            case '!':
                                ...
                                break;
                            case '<':
                                tag.type = Tag::Type::InheritanceBegin;
                                break;
                            case '$':
                                tag.type = Tag::Type::ContentBegin;
                                break;
                            case '@':
                                tag.type = Tag::Type::LambdaBegin;
                                break;
                            default:
                                tag.type = Tag::Type::Variable;
                                break;
                ...
                WalkControl renderComponent(const RenderHandler &handler, context_internal &ctx, component &comp) {
                    ...
                    switch (tag.type) {
                        ...
                        case Tag::Type::UnescapedVariable:
                            // if var exists, get variable by that name
                            // context_internal:setdelimiter  .  context:base_context.get  .  get from context stack
                            if ((var = ctx.ctx.get(tag.name)) != nullptr) {
                                // render variable
                                if (!renderVariable(handler, var, ctx, tag.type == Tag::Type::Variable)) {
                                    // stop if it failed somehow
                                    return WalkControl::Stop;
                                }
                            }
                            break;
                        case Tag::Type::SectionBegin:
                            // if var has a valid name
                            if ((var = ctx.ctx.get(tag.name)) != nullptr) {
                                // if it's lambda
                                if (var->is_lambda() || var->is_lambda2()) {
                                    // render lambda
                                    if (!renderLambda(handler, var, ctx, RenderLambdaEscape::Optional,
                                                      *comp.tag.sectionText, true)) {
                                        return WalkControl::Stop;
                                    }
                                // else, render section
                        ...
                        case Tag::Type::SectionBeginInverted:
                            // if var doesn't exist, is false or empty
                            if ((var = ctx.ctx.get(tag.name)) == nullptr || var->is_false() || var->is_empty_list()) {
                                // render section normally
                                // comp = current component node
                                // var =
                                renderSection(handler, ctx, comp, var);
                            }
                            return WalkControl::Skip;
                        case Tag::Type::ContentBegin:
                            // if this content was inherited from any other template (look for the most recent one)
                            {
                                typename std::vector<std::unordered_map<string_type,component>*>::reverse_iterator rit = ctx.inherited_content.rbegin();
                                // for all inherited context
                                std::unordered_map<string_type,component>* current_context = nullptr;
                                while (rit!=ctx.inherited_content.rend()){
                                    // current inherited context
                                    current_context = *rit;
                                    if (current_context->find(tag.name) != current_context->end()){
                                          break;
                                    }
                                    ++rit;
                                }
                                if (rit!=ctx.inherited_content.rend()){
                                    //renderSection(handler, ctx, *((*rit)->find(tag.name)), ctx.ctx.get(tag.name));
                                    renderSection(handler, ctx, current_context->find(tag.name)->second, var);
                                } else {
                                    // we interpret the original as a section
                                    //renderSection(handler, ctx, comp, ctx.ctx.get(tag.name));
                                    renderSection(handler, ctx, comp, var);
                                }
                                return WalkControl::Skip;
                            }
                        case Tag::Type::InheritanceBegin:
                            // if partial exists with that name as partial or string
                            if ((var = ctx.ctx.get_partial(tag.name)) != nullptr && (var->is_partial() || var->is_string())) {
                                // get function result or the string
                                const string_type partial_result = var->is_partial() ? var->partial_value()() : var->string_value();
                                // create a template with the partial
                                basic_mustache tmpl{partial_result};
                                // set the context escape handler to the partial template
                                ...
                                if (!tmpl.is_valid()) {
                                    ...
                                    // get content to be inherited
                                    std::unordered_map<string_type,component> content_to_be_inherited;
                                    // for each child of this component of the derived template
                                    for (component &child : comp.children) {
                                        // if it's content definition
                                        if (child.tag.type == Tag::Type::ContentBegin){
                                            // save this content
                                            content_to_be_inherited[child.tag.name] = child;
                                        }
                                    }
                                    // this variable with the contents is inserted in the base template
                                    ctx.inherited_content.push_back(&content_to_be_inherited);
                                    // base template is rendered with this new context
                                    tmpl.render(handler, ctx);
                                    // we remove the inherited content from the context
                                    ctx.inherited_content.pop_back();
                                    // if base template is not valid now
                                    if (!tmpl.is_valid()) {
                                        errorMessage_ = tmpl.error_message();
                                        return WalkControl::Stop;
                                    } else {
                        ...
                        case Tag::Type::Partial:
                            if ((var = ctx.ctx.get_partial(tag.name)) != nullptr &&
                                ...
                                const auto partial_result = var->is_partial() ? var->partial_value()()
                                                                              : var->string_value();
                                basic_mustache tmpl{partial_result};
                                tmpl.set_custom_escape(escape_);
                                if (!tmpl.is_valid()) {
                ...
                bool renderLambda(const RenderHandler &handler,
                                  const basic_data<string_type> *var,
                                  context_internal &ctx,
                                  RenderLambdaEscape escape,
                                  const string_type &text,
                                  bool parseWithSameContext) {
                    const typename  renderer_internal::type2 render2 = [this, &handler, var, &ctx, parseWithSameContext, escape](
                            const string_type &text,
                            bool escaped) {
                    ...
                    const typename renderer_internal::type1 render = [&render2](const string_type &text) {
                        return render2(text, false);
                    };
                    if (var->is_lambda2()) {
                        renderer_internal renderer{render, render2, &ctx};
                ...
                bool renderVariable(const RenderHandler &handler,
                                    const basic_data<string_type> *var,
                                    context_internal &ctx,
                                    bool escaped) {
                    if (var->is_string()) {
                        ...
                        const RenderLambdaEscape escapeOpt = escaped ? RenderLambdaEscape::Escape
                                                                     : RenderLambdaEscape::Unescape;
                ...
                void renderSection(const RenderHandler &handler,
                                   context_internal &ctx,
                                   component &incomp,
                                   const basic_data<string_type> *var) {
        ...
        using renderer_internal = mustache::renderer_internal;
