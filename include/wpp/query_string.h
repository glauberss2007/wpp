...
inline CHAR qs_hex2dec(CHAR x){
    return (((x)>='0'&&(x)<='9') ? (x)-48 : ((x)>='A'&&(x)<='F') ? (x)-55 : ((x)>='a'&&(x)<='f') ? (x)-87 : 0);
}
...
inline int qs_strncmp(const char * s, const char * qs, size_t n)
{
    ...
    while(n-- > 0)
    {
        ...
        if ( ! qs_is_qschar(u1) ) {  u1 = '\0';  }
        if ( ! qs_is_qschar(u2) ) {  u2 = '\0';  }
...
//    size_t skip;
    ...
    char * const * low =  std::lower_bound(qs_kv, qs_kv+qs_kv_size, key, [](char const * c1, char const * c2){return strncmp(c1, c2, std::strlen(c1)) < 0;});
