...
namespace wpp {
    namespace db {
        ...
        ///////////////////////////////////////////////////////////////
        // Bindings should work for arithmetic types and std::strings
        template<class T>
        struct is_c_str
        ...
        class data_object_statement {
            public:
                ...
                bool bind_param(P param_no_or_name, const char (&parameter)[N]) {
                    // Temporary std::string literals, however, have to be copied
                    return this->bind_value<P, std::string>(param_no_or_name, parameter);
                };

                template<typename P, typename T>
                bool bind_param(P param_no_or_name, const T &&param) {
                    // We cannot bind r-value references that represent temporaries
                    // Thus, the default case is to bind the variable value
                    const T param_copy = param;
                    return this->bind_value(param_no_or_name, param_copy);
                }


                // Binds a std::string literal value to a position
                template<typename P, int N>
            ...
            protected:
                ...
                // Aux for bind_param and bind_column
                template<typename P, typename T>
                int register_bound_param(P param_no_or_name, T &parameter, const bool is_param, const bool make_copy) {
                    ...
                    if (make_copy) {
                        // Save copy of data as a std::string
                        ...
                        // param.data doesn't need to be set because we'll use the external value
                ...
                // pointer to something convertible to std::string by the methods above
                template<typename T>
        ...
        class data_object_crtp
            ...
            public:
                ...
                virtual std::shared_ptr<data_object_statement>
                prepare(std::string statement, std::unordered_map<attribute_type, driver_option> options = {}) override {
                ...
                //
                //virtual int
                //preparer(const std::string sql, std::shared_ptr<data_object_statement> &stmt, std::unordered_map<attribute_type, driver_option> driver_options = {}) override {
                //    data_object::raise_impl_error(this, nullptr, "IM001", "driver does not support preparer");
                //    return false;
                //}
                //
                //virtual long doer(const std::string sql) override {
                //    data_object::raise_impl_error(this, nullptr, "IM001", "driver does not support doer(const std::string sql)");
                //    return false;
                //}
                //
                //virtual bool quoter(std::string unquoted,
                ...
                //    data_object::raise_impl_error(this, nullptr, "IM001", "driver does not support quoting");
                //    quoted = "";
                //    return false;
                //};
                //
                ...
                //    /* TODO: this should be an exception; see the auto-commit mode comments below */
                //    throw std::logic_error("This driver doesn't support transactions");
                //    return false;
                //}
                //
                ...
                //    data_object::raise_impl_error(this, nullptr, "IM001", "driver does not support persistent_shutdown");
                //}
                //
        ...
        int data_object_statement::bind_input_parameters(std::vector<std::pair<std::string, std::string>> &input_params) {
        ...
        int data_object_statement::emulate_parameter_binding() {
            ...
            // this->_active_query_string holds the query with binds expanded and quoted.

            /* std::string is leftover from previous calls so data_object_statement::debug_dump_params() can access */
        ...
        int data_object_statement::really_register_bound_param(bound_param_data &param, int is_param) {
            ...
            std::unordered_map<std::string, bound_param_data> *hash = is_param ? &this->_bound_param : &this->_bound_columns;

            if (is_column && !param.name.empty() && !this->_columns.empty()) {
                this->map_the_name_to_column(param);
            }

            // Add placeholder syntax to name
            if (!param.name.empty()) {
            ...
            //try to rewrite name -> position
            if (is_param && !this->rewrite_name_to_position(param)) {
                param.name.clear();
                return 0;
            }

            // driver can perform normalization
        ...
        int data_object_statement::parse_params(std::string &inquery, std::string &outquery) {
            ...
            if (placeholders_do_not_match_parameters) {
                ...
                if (query_type != PLACEHOLDER_POSITIONAL && bindno > this->_bound_param.size()) {
                    ...
                    for (plc = 0; plc < placeholders.size(); ++plc) {
                        // bound_param_data param = this->_bound_param
            ...
            if (this->_supports_placeholders == placeholder_support::PLACEHOLDER_NONE) {
                ...
                do {
                    // position of the placeholder in the query
                ...
                //if (this->_bound_param_map.empty()) {
                //    ALLOC_HASHTABLE(this->_bound_param_map);
                //    hash_init(this->_bound_param_map, 13, nullptr, free_param_name, 0);
                //}

                for (plc = 0; plc < placeholders.size(); ++plc) {
                    std::string name = placeholders[plc].unquoted;
                    //std::string name = placeholders[plc].unquoted;
                    //hash_index_update_mem(this->_bound_param, placeholders[plc].bindno, name, placeholders[plc].len + 1);
                    //efree(name);
        ...
        int data_object_statement::param_hook(bound_param_data &attr, param_event val) {
            // Driver does not implement param_hook, but that's OK
            return 0;
        }

        int data_object_statement::dispatch_param_event(param_event event_type) {
            for (auto &&item : this->_bound_param) {
                bound_param_data &param = item.second; // get the parameter
