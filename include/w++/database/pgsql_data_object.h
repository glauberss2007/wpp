    ...
    namespace db {
        ...
        class pgsql_statement
            ...
            public:
                ...
                // pdo_stmt_dtor_func, dtor; pgsql_stmt_dtor
                ~pgsql_statement();

                // pdo_stmt_execute_func		executer; pgsql_stmt_execute
                virtual int executer() override;

                // pdo_stmt_fetch_func, fetcher; pgsql_stmt_fetch
                virtual int fetcher(fetch_orientation ori, long offset) override;

                // pdo_stmt_describe_col_func	describer; pgsql_stmt_describe
                virtual int describer(int colno) override;

                // pdo_stmt_get_col_data_func	get_col; pgsql_stmt_get_col
                virtual int get_col(int colno, std::string &ptr, int &caller_frees) override;

                // pdo_stmt_param_hook_func	param_hook; pgsql_stmt_param_hook
                virtual int param_hook(bound_param_data &attr, param_event val) override;

                // pdo_stmt_set_attr_func, set_attribute; NULL
                //virtual int set_attribute(long attr, std::string &val) override;

                // pdo_stmt_get_attr_func, get_attribute; NULL
                //virtual int get_attribute(long attr, std::string &val) override;

                // pdo_stmt_get_column_meta_func get_column_meta; pgsql_stmt_get_column_meta
                virtual int get_column_meta(long colno, column_data &return_value) override;

                // pdo_stmt_next_rowset_func, next_rowset; NULL
                //virtual int next_rowset_func() override;

                // pdo_stmt_cursor_closer_func 	cursor_closer;  pgsql_statement_cursor_closer
        ...
        class pgsql_data_object
            ...
            public:
                ...
                pgsql_data_object(std::string data_source, std::string username, std::string passwd, std::unordered_map<attribute_type, driver_option> options = {}):
                ...
                // pgsql_handle_closer, closer; pdo_dbh_close_func;
                ~pgsql_data_object();

                virtual int handle_factory(std::unordered_map<attribute_type, driver_option> options = {}) override;

                // pgsql_handle_preparer, preparer; pdo_dbh_prepare_func;
                virtual int
                ...
                // pgsql_handle_doer, doer; pdo_dbh_do_func;
                virtual long doer(const std::string sql) override;

                // pgsql_handle_quoter, quoter; pdo_dbh_quote_func;
                virtual bool quoter(std::string unquoted,
                                    std::string &quoted,
                                    param_type paramtype) override;

                // pgsql_handle_begin, begin; pdo_dbh_txn_func;
                virtual int begin() override;

                // pgsql_handle_commit, commit; pdo_dbh_txn_func;
                virtual int commit_func() override;

                // pgsql_handle_rollback, rollback; pdo_dbh_txn_func;
                virtual int rollback() override;

                // pdo_pgsql_set_attr, set_attribute; pdo_dbh_set_attr_func;
                virtual int set_attribute_func(long attr, const driver_option &val) override;

                // pdo_pgsql_last_insert_id, last_id; pdo_dbh_last_id_func;
                virtual std::string last_id(const std::string name) override;

                // pdo_pgsql_fetch_error_func, fetch_err; pdo_dbh_fetch_error_func;
                virtual int fetch_err(const data_object_statement *stmt, std::vector<std::string> &info) override;

                // pdo_pgsql_get_attribute, get_attribute; pdo_dbh_get_attr_func;
                virtual int get_attribute(long attr, driver_option &val) override;

                // pdo_pgsql_check_liveness,	/* check_liveness */ check_liveness; pdo_dbh_check_liveness_func;
                virtual int check_liveness() override;

                // ", persistent_shutdown; pdo_dbh_request_shutdown;
                //virtual void persistent_shutdown() override;

                // pgsql_handle_in_transaction, in_transaction; pdo_dbh_txn_func;
        ...
        pgsql_statement::~pgsql_statement() {
            //bool server_obj_usable = !Z_ISUNDEF(this->_database_object_handle)
            //                              && IS_OBJ_VALID(EG(objects_store).object_buckets[Z_OBJ_HANDLE(this->_database_object_handle)])
            //                              && !(GC_FLAGS(Z_OBJ(this->_database_object_handle)) & IS_OBJ_FREE_CALLED);
        ...
        int pgsql_statement::prepare(){
            ...
            do {
                stmt_retry = false;
                /* we deferred the prepare until now, because we didn't know anything about the parameter types; now we do */
        ...
        int pgsql_statement::executer() {
            ...
            if (statement_has_cursor) {
                if (!this->get_cursor_result()) return 0;
            } else if (!this->_stmt_name.empty()) {
                if (!this->_is_prepared) {
                    if (!this->prepare()) return 0;
                } else {
                }
                if (!this->execute_prepared()){ return 0; }
            } else if (this->_supports_placeholders == PLACEHOLDER_NAMED) {
                if (!this->execute_with_param()){ return 0; }
            } else {
                if (!this->execute_plain_query()){ return 0; }
            }
            ExecStatusType status;
            if (!this->check_status(status)) {return 0;}
        ...
        int pgsql_statement::get_col(int colno, std::string &ptr, int &caller_frees) {
            ...
            if (PQgetisnull(this->_result, this->_current_row - 1, colno)) { /* Check if we got nullptr */
                ptr.clear();
            } else {
        ...
        int pgsql_statement::param_hook(bound_param_data &param, param_event event_type) {
            ...
            if (named_placeholders) {
                switch (event_type) {
                    case PARAM_EVT_FREE:
                        break;
                    case PARAM_EVT_NORMALIZE: // from $1, $2 into 0, 1 etc....
                        if (!param.name.empty()) {
                    ...
                    case PARAM_EVT_ALLOC:
                        ...
                        //if (!zend_hash_index_exists(this->_bound_param_map, param.paramno)) {
                    ...
                    case PARAM_EVT_EXEC_PRE: // pre-process for executing the query
                        if (this->_bound_param_map.empty()) { // if there are no parameters to bind
                            return 1;
                        }
                        if (this->_param_values.empty()) { // if we haven't allocated space for the parameter real std::string values yet
                        ...
                        if (param.paramno >= 0) { // if it's a named parameter
                ...
                if (param.param_type == PARAM_BOOL
                    //&& ((param.param_type & PARAM_INPUT_OUTPUT) != PARAM_INPUT_OUTPUT)
                        ) {
                    const std::string s = ((*((std::string*) param.parameter))[0] == 't' || (*((std::string*) param.parameter))[0] == '0') ? "t" : "f";
        ...
        // pdo_stmt_set_attr_func, set_attribute; NULL
        //int pgsql_statement::set_attribute(long attr, std::string &val){
        // }

        // pdo_stmt_get_attr_func, get_attribute; NULL
        //int pgsql_statement::get_attribute(long attr, std::string &val){
        // }
        ...
        // pdo_stmt_next_rowset_func, next_rowset; nullptr
        //int pgsql_statement::next_rowset_func(){
        //    return 1;
        //}
        ...
        int pgsql_data_object::handle_factory(std::unordered_map<attribute_type, driver_option> driver_options) {
            ...
            /* PostgreSQL wants params in the connect std::string to be separated by spaces,
            ...
            /* support both full connection std::string & connection std::string + login and/or password */
            if (!tmp_user.empty() && !tmp_pass.empty()) {
                conn_str = this->_data_source + " user='"+tmp_user+"' password='"+tmp_pass+"' connect_timeout=" +std::to_string(connect_timeout);
            } else if (!tmp_user.empty()) {
                conn_str = this->_data_source + " user='"+tmp_user+"' connect_timeout=" +std::to_string(connect_timeout);
            } else if (!tmp_pass.empty()) {
                conn_str = this->_data_source + " password='"+tmp_pass+"' connect_timeout=" +std::to_string(connect_timeout);
            } else {
            ...
            //PQsetNoticeProcessor(this->_server, (void(*)(void*,const char*))_pdo_pgsql_notice, (void *)&dbh);
        ...
        int pgsql_data_object::preparer(std::string sql,
            ...
            if (scrollable) {
                ...
                if ((driver_options.count(ATTR_EMULATE_PREPARES) ? driver_options[ATTR_EMULATE_PREPARES].get_int() : this->_emulate_prepares) == 1){
                    emulate = 1;
                }
                if ((driver_options.count((wpp::db::attribute_type) PGSQL_ATTR_DISABLE_PREPARES) ? driver_options[(wpp::db::attribute_type) PGSQL_ATTR_DISABLE_PREPARES].get_int() : this->_disable_prepares) == 1){
                    execute_only = 1;
                }
            } else {
        ...
        long pgsql_data_object::doer(const std::string sql) {
            ...
            if (qs != PGRES_COMMAND_OK && qs != PGRES_TUPLES_OK) {
                pgsql_data_object::pgsql_error(this, nullptr, qs, PQresultErrorField(res, PG_DIAG_SQLSTATE), "", __FILE__, __LINE__);
                PQclear(res);
                return -1;
            }
        ...
        bool pgsql_data_object::quoter(std::string unquoted,
            ...
            switch (paramtype) {
                case PARAM_LOB:
                    ...
                    escaped = PQescapeByteaConn(this->_server, (unsigned char *)unquoted.data(), unquoted.size(), &tmp_len);
                ...
                default:
                    char* quoted_char;
                    size_t quotedlen = PQescapeStringConn(this->_server, quoted_char, unquoted.data(), unquoted.length(), nullptr);
        ...
        std::string pgsql_data_object::last_id(const std::string name) {
            ...
            if (res && (status == PGRES_TUPLES_OK)) {
                ...
                pgsql_data_object::pgsql_error(this, nullptr, status, PQresultErrorField(res, PG_DIAG_SQLSTATE), "", __FILE__, __LINE__);
            }
        ...
        int pgsql_data_object::get_attribute(long attr, driver_option &val) {
            ...
            switch (attr) {
                ...
                case ATTR_SERVER_INFO: {
                    ...
                    return_value = "PID: " + std::to_string(spid) + "; Client Encoding: " + (char *) PQparameterStatus(this->_server, "client_encoding") + "; Is Superuser: " + (char *) PQparameterStatus(this->_server, "is_superuser") + "; Session Authorization: " + (char *) PQparameterStatus(this->_server, "session_authorization") +"; Date Style: " + (char *) PQparameterStatus(this->_server, "DateStyle");
                }
                    break;
        ...
        // nullptr, persistent_shutdown; pdo_dbh_request_shutdown
        //void pgsql_data_object::persistent_shutdown() {
        //    return;
        //}
        ...
        int pgsql_data_object::transaction_cmd(const std::string cmd) {
            ...
            if (PQresultStatus(res) != PGRES_COMMAND_OK) {
                pgsql_data_object::pgsql_error(this, nullptr, PQresultStatus(res), PQresultErrorField(res, PG_DIAG_SQLSTATE), "", __FILE__, __LINE__);
                ret = 0;
            }
