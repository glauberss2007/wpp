                        ...
                        switch (param.param_type) {
                            ...
                            case PARAM_BOOL:
                                if (std::numeric_limits<long>::max() > 2147483647) {
                                    if (SQLITE_OK == sqlite3_bind_int64(this->_stmt, param.paramno + 1, * (long *) param.parameter)) {
                                        return 1;
                                    }
                                } else {
                                    if (SQLITE_OK == sqlite3_bind_int(this->_stmt, param.paramno + 1, * (long *) param.parameter)) {
                                        return 1;
                                    }
                                }
                                sqlite_data_object::sqlite_error(this->_H, this, __FILE__, __LINE__);
                                return 0;
                            case PARAM_FLOAT:
                                if (SQLITE_OK == sqlite3_bind_double(this->_stmt, param.paramno + 1, * (double *) param.parameter)) {
                                    return 1;
                                }
                                return 0;
                            case PARAM_LOB:
