...
namespace SimpleWeb {
    ...
    class ServerBase {
        ...
        public:
            class Response
                    : public std::enable_shared_from_this<Response>, public std::ostream {
                    ...
                    Response(std::shared_ptr<Session> session, long timeout_content) noexcept : std::ostream(
                            &streambuf), session(std::move(session)), timeout_content(timeout_content) {}
                ...
                public:
                    ...
                    /// Convenience function for writing success status line, header fields, and content
                    void
                    write(std::istream &content, const CaseInsensitiveMultimap &header = CaseInsensitiveMultimap()) {
                        write(StatusCode::success_ok, content, header);
                    }
            ...
            class Request {
                ...
                private:
                    ...
                    Request(const std::string &remote_endpoint_address = std::string(),
                            unsigned short remote_endpoint_port = 0) noexcept
                            : content(streambuf), remote_endpoint_address(remote_endpoint_address),
        ...
        protected:
            class Connection
                ...
                public:
                    ...
                    Connection(std::shared_ptr<ScopeRunner> handler_runner, Args &&... args) noexcept : handler_runner(
                            std::move(handler_runner)), socket(new socket_type(std::forward<Args>(args)...)) {}
                    ...
                    Session(std::shared_ptr<Connection> connection) noexcept : connection(std::move(connection)) {
                        try {
                            ...
                            request = std::shared_ptr<Request>(
                                    new Request(remote_endpoint.address().to_string(), remote_endpoint.port()));
                        }
                        catch (...) {
